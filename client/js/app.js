...
function getPriorityColor(priority) {
  switch (priority?.toLowerCase()) {
    case 'high': return 'priority-high priority-badge-high';
    case 'low': return 'priority-low priority-badge-low';
    default: return 'priority-medium priority-badge-medium';
  }
}
...
function renderColumns() {
  ...
  board.innerHTML = columns.map(col => 
    '<div class="column ' + col.color + ' rounded-lg p-4" data-column="' + col.id + '" ondrop="drop(event)" ondragover="allowDrop(event)" ondragleave="dragLeave(event)">' +
      ...
      taskList.innerHTML = columnTasks.map(task => 
        '<div class="task-card bg-gray-800 rounded p-4 cursor-move border-l-4 ' + getPriorityColor(task.priority) + '" draggable="true" ondragstart="drag(event)" ondragend="dragEnd(event)" data-id="' + task.id + '">' +
          ...
          '<div class="flex justify-between items-center mt-2">' +
            '<span class="text-xs px-2 py-1 rounded ' + getPriorityColor(task.priority).split(' ')[1] + ' text-white">' + (task.priority || 'medium') + '</span>' +
